...
{
    float dissolve = SimpleNoise(input.uv, _NoiseScale);

    clip(_Opacity - dissolve);
    ...
    half4 emission = step(_Opacity - _EmissionWidth, dissolve) * _EmissionColor;

    color.rgb += emission.rgb * _TotalBrightness;
