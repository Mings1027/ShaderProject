...
float DitherPattern(float2 screenPos, float ditherSize)
{
    float2 screenPosScaled = screenPos * _ScreenParams.xy / ditherSize;
    ...
    uint x = uint(screenPosScaled.x) % 4;
    uint y = uint(screenPosScaled.y) % 4;
...
half4 LitPassFragment(Varyings input) : SV_Target
{
    ...
    float ditherThreshold = DitherPattern(screenPos, _DitherSize);

    clip(_Opacity - ditherThreshold);
