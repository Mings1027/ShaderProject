...
namespace ShaderCode.FPL
{
    public class FakeLight2DEditor : ShaderGUI
    {
        ...
        {
            public static readonly string[] queueControlNames = Enum.GetNames (typeof (CustomMaterialEditor.QueueControl));
        ...
        public virtual void DrawLightModules(Material material)
        {
            ...
            for (int i = 0; i < ModulesProperties.Count; i++)
            {
                ...
                if (ModulesProperties[i].name == "_Halo")
                {
                    if (ModulesProperties[i].floatValue == 0) i += 4; //skip 4
                    continue;
                }

                if (ModulesProperties[i].name == "_DistanceFade")
                {
